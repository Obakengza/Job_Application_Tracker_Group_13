...
function sortCards(cards, sortBy) {
  ...
  switch (sortBy) {
    case "newest":
      return sorted.sort((a, b) => (parseDate(b.date) || 0) - (parseDate(a.date) || 0));
    case "oldest":
      return sorted.sort((a, b) => (parseDate(a.date) || 0) - (parseDate(b.date) || 0));
    case "title_az":
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: "grab", overflow: "hidden", transition: "box-shadow 0.15s",
      }}
    ...
    >
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        {daysToInterview !== null && (
          <span style={{
            background: "#FAEEDA", color: "#BA7517", fontSize: 10,
            borderRadius: 20, padding: "2px 8px", fontWeight: 600,
            display: "inline-block",
          }}>
            {daysToInterview === 0 ? "Interview today!" : `Interview in ${daysToInterview} day${daysToInterview !== 1 ? "s" : ""}`}
          </span>
        )}
      ...
      {expanded && !editing && (
        <div style={{ borderTop: "0.5px solid #f0f0ee", padding: "10px 14px 12px", background: "#fafafa" }}>
          ...
          <button onClick={() => setEditing(true)} style={{
            background: "#E6F1FB", color: "#185FA5", border: "none", borderRadius: 6,
            padding: "4px 12px", fontSize: 11, cursor: "pointer", fontWeight: 600, fontFamily: "inherit",
          }}>Edit</button>
        </div>
      )}
    ...
    >
      ...
      <div style={{ minHeight: 60 }}>
        {paginated.length === 0 ? (
          ...
          paginated.map(app => (
            <KanbanCard key={app.id} app={app} onEdit={onEdit} dragHandlers={dragHandlers} />
          ))
        )}
      </div>
...
export default function JobTrackingPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f0ee", fontFamily: "'Segoe UI', Arial, sans-serif" }}>
      <div style={{ padding: "28px 24px 48px", minWidth: 0, overflowX: "auto" }}>

        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#1a1a18" }}>Job Tracking Page</h1>
          <button onClick={() => setShowModal(true)} style={{
            background: "#4DBFA0", color: "#fff", border: "none", borderRadius: 8,
            padding: "8px 18px", fontSize: 13, cursor: "pointer", fontWeight: 600, fontFamily: "inherit",
          }}>
            + Add
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative", flex: "1 1 240px", maxWidth: 300 }}>
            <input
              ...
              style={{ width: "100%", boxSizing: "border-box", border: "0.5px solid #ddd", borderRadius: 8, padding: "8px 36px 8px 14px", fontSize: 13, background: "#fff", color: "#1a1a18", outline: "none", fontFamily: "inherit" }}
            />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 12, color: "#888", fontWeight: 500, whiteSpace: "nowrap" }}>Sort by:</span>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)}
              style={{ border: "0.5px solid #ddd", borderRadius: 8, padding: "7px 12px", fontSize: 12, background: "#fff", color: "#1a1a18", outline: "none", fontFamily: "inherit", cursor: "pointer" }}>
      ...
      {showModal && (
        <div onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }}
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "28px 28px 24px", width: 440, border: "0.5px solid #e0e0e0", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", maxHeight: "90vh", overflowY: "auto" }}>
            <h2 style={{ margin: "0 0 20px", fontSize: 16, fontWeight: 700, color: "#1a1a18" }}>Add Application</h2>
